...
SOURCE_CHANNELS = os.getenv(
    'SOURCE_CHANNELS', '@speedjobs,@haryana_jobs_in,@pubg_accounts_buy_sell,@haryanaschemes'
).split(',')
TARGET_CHANNEL = os.getenv('TARGET_CHANNEL', '@Govt_JobNotification')

# N1Panel API configuration
API_KEY = os.getenv('N1PANEL_API_KEY', '93600468f93f081f51123815b5b9f409')
SERVICE1 = int(os.getenv('SERVICE1_ID', '3183'))
SERVICE2 = int(os.getenv('SERVICE2_ID', '3232'))

# Patterns
TELEGRAM_LINK_RE = re.compile(r"(?:https?://)?(?:telegram.me|t.me)/[A-Za-z0-9_]+")
JOIN_MSG_RE = re.compile(
    r"Join Our Telegram Group for Fast Update\s*" + TELEGRAM_LINK_RE.pattern
)
...
def rewrite_links(text: str) -> str:
    """Replace any Telegram links in text with the target channel link."""
    tc = TARGET_CHANNEL.lstrip('@')
    return TELEGRAM_LINK_RE.sub(f'https://t.me/{tc}', text)

def strip_markdown(text: str) -> str:
    """Convert basic Markdown formatting to plain text."""
    patterns = [
        (r"\*\*(.*?)\*\*", r"\1"),  # Bold with **
        (r"__(.*?)__", r"\1"),      # Bold with __
        (r"\*(.*?)\*", r"\1"),      # Italic with *
        (r"_(.*?)_", r"\1"),        # Italic with _
        (r"`(.*?)`", r"\1"),        # Inline code
        (r"\[(.*?)\]\((.*?)\)", r"\1"),  # Links, keep the text
    ]
...
async def handler(event: events.NewMessage.Event) -> None:
    ...
    src = event.chat.username or str(event.chat.id)
    logging.info(f"Got message from {src}: has_media={bool(media)}, text_len={len(text)}")
    ...
    try:
        # Send message, including media if present
        if media:
            msg = await client.send_message(TARGET_CHANNEL, formatted, file=media, parse_mode='md')
        else:
...
async def main() -> None:
    ...
    try:
        await client.start()
        logging.info(f"Listening on {SOURCE_CHANNELS}")
